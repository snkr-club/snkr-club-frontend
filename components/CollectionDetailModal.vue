<template>
    <v-dialog v-model="dialogModel" max-width="800" :persistent="collectionStore.updateLoading">
        <v-card>
            ...
            <v-card-actions>
                ...
                <v-btn
                    :disabled="!valid"
                    @click="save"
                    :loading="collectionStore.updateLoading"
                >
                    SALVEAZA
                </v-btn>
                <v-btn
                    @click="emit('dialogClosed')"
                    :disabled="collectionStore.updateLoading"
                >
                    ANULEAZA
                </v-btn>
            </v-card-actions>
...
const categoryId = ref(props.selectedItem.categoryId)
...
const save = async () => {
    ...
    await collectionStore.update(temp)
    emit("dialogClosed")
}
